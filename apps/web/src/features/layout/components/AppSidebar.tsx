...
export function AppSidebar({
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 hidden bg-gradient-to-br from-[#0aa0ed] via-[#056ba6] to-[#072d4a] text-white lg:flex lg:flex-col transition-all duration-300 ease-in-out z-20 ${isCompact ? "w-[88px]" : "w-[270px]"}`}>
      <button
      ...
      <div className={`px-7 py-8 transition-all duration-300 ${isCompact ? "flex justify-center px-0" : "flex flex-col"}`}>
        ...
        {!isCompact && <p className="ml-[64px] mt-1 text-[10px] font-bold uppercase tracking-[0.2em] text-[#063b5f] whitespace-nowrap overflow-hidden">Software Contable</p>}
      </div>
      ...
      <nav className="mt-4 flex-1 space-y-2 px-4 overflow-hidden">
        {getNavItems(user.role).map((item) => {
          ...
          return (
            <NavLink
              className={({ isActive }) =>
                `flex h-12 items-center rounded-md text-left text-sm font-medium transition-all ${isCompact ? "w-12 justify-center px-0 mx-auto" : "w-full gap-4 px-4"} ${
      ...
      <div className={`border-t border-white/20 py-6 transition-all ${isCompact ? "px-0 flex justify-center" : "px-5"}`}>
        <div className="flex flex-col gap-4 w-full">
          <button onClick={onLogout} className={`flex items-center text-white/90 hover:bg-white/10 hover:text-white rounded-lg transition-colors ${isCompact ? "justify-center w-12 h-12 mx-auto" : "gap-3 w-full p-2"}`} title="Cerrar sesion">
            <LogOut size={26} className="shrink-0" />
            {!isCompact && <span className="font-medium whitespace-nowrap">Cerrar sesion</span>}
          </button>
          {!isCompact && <div className="text-center text-xs text-white/50 font-medium tracking-wide">Ultima version v0.1.15</div>}
        </div>
      </div>
    </aside>
  );
