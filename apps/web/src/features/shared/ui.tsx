...
export function SearchBox({
  ...
  return (
    <label className="relative block w-full">
      ...
      <input className="h-11 w-full rounded-md border border-[#c9dbef] bg-white pl-12 pr-10 text-sm outline-none focus:border-[#0aa0ed]" onChange={(event) => onChange(event.target.value)} placeholder={placeholder} value={value} />
      {value && (
        <button
...
export function IconButton({
  ...
  return (
    <button className={`flex h-10 w-10 items-center justify-center rounded-md border bg-white disabled:opacity-50 ${tones[tone]}`} disabled={disabled} onClick={onClick} title={label} type="button">
      <Icon size={18} />
    </button>
  );
}
